...
bootstrap_param_est_CH <- function(wide, b, days=paste0("D",1:28), bin=rep(1,length(days)-1), 
           Em=get_emission(wide, days), tol=1E-6, maxiter=200, silent=FALSE, strt) {
    ...
    fit=niaidMI:::.baum_welsh(Pri=list(strt[[2]]), s=s, Tran=strt[[1]], Em=Em, bin=bin, tol=tol,  maxiter=maxiter)
...
enc_imputeBS_CH <- function(data, q_func, bin, start_initP, start_tP, m,
                            tol_llk = 0, fix_const = 1E-6, dataNM) { #m: num of bootstrap
  ...
  #strt = niaidMI:::.get_start(M=M,s=s,bin=bin)
  strt = get_start(data = data, bin = bin)
  
  for (i in 1:m) {
    #fitMC <- BW_CH(cart[[i]], q_func, bin, start_initP, start_tP, tol_llk = tol_llk, message = FALSE)
    
    ### for debug purpose ######################################################
    fitMC <- bootstrap_param_est_CH(wide=cartNM[[i]], b=0, bin=bin, tol = tol_llk, silent = TRUE, maxiter = 100000000000, strt = strt)$fit
    fitMC <- list(fitMC[[2]][[1]], fitMC[[1]])
    ### for debug purpose ######################################################
...
imputeBS_CH <- function(data, q_func, bin, start_initP, start_tP, m,
                        by = NULL, tol_llk = 0, fix_const = 1E-6, dataNM) {
  if (is.null(by)) {
    result <- enc_imputeBS_CH(data, q_func, bin, start_initP, start_tP, m, tol_llk, fix_const, dataNM)
    return(result)
  }
